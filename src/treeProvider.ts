...
import {
    debounce,
    throttle,
} from "./kernels/git/decorators"

import {
    API as GitAPI,
    Repository as GitAPIRepository,
    Ref,
    RefType,
} from "./kernels/git/api/git"

import {
    Git,
    Repository,
} from "./kernels/git/git"

import {
    anyEvent,
    filterEvent,
    eventToPromise,
} from "./kernels/git/util"

import { NAMESPACE } from "./kernels/constants"
import { 
    normalizePath,
    getDefaultBranch,
...
class FileElement implements DiffStatusInterface {
    constructor(
...
export class GitTreeCompareProvider implements TreeDataProvider<Element>, Disposable {
    ...
    // Diff results
    private filesInsideTreeRoot: Map<FolderAbsPath, DiffStatusInterface[]>;
    private filesOutsideTreeRoot: Map<FolderAbsPath, DiffStatusInterface[]>;
    ...
    private async updateDiff(fireChangeEvents: boolean) {
        ...
        const filesInsideTreeRoot = new Map<FolderAbsPath, DiffStatusInterface[]>();
        const filesOutsideTreeRoot = new Map<FolderAbsPath, DiffStatusInterface[]>();
    ...
    private getDiffStatus(fileEntry?: FileElement): DiffStatusInterface | undefined {
        if (fileEntry) {
            return fileEntry;
        }
    ...
    async discardChanges(entry?: FileElement | FolderElement) {
        let statuses: DiffStatusInterface[] = [];
    ...
    async doDiscardChanges(statuses: DiffStatusInterface[]) {
